...
// testrunner_vhdl_syntax.exe --log_level=all -- --param1=1 --param2=2
...
public:
    ...
    {
        BOOST_TEST_INFO("constructor dataset_loader(std::initializer_list<std::string>)");
    }
    dataset_loader(std::string s1, std::string s2)
    : m_input_extension{s1, s2}
    {
        BOOST_TEST_INFO("constructor dataset_loader(std::string s1, std::string s2)");
    }
...
{
    BOOST_TEST_INFO("constructor dataset_loader()");

    unsigned argc = boost::unit_test::framework::master_test_suite().argc;
