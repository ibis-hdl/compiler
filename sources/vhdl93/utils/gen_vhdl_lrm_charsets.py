...
import collections
import unicodedata
 # XXX ftp://ftp.unicode.org/Public/3.0-Update/UnicodeData-3.0.0.html#General Category
 # https://stackoverflow.com/questions/14245893/efficiently-list-all-characters-in-a-given-unicode-category
import json # for escaping
...
class Iso8859_1_Gen:
    ...
    def __init__(self):
        for line in [l for l in iso8859_1_table.splitlines() if l.split()]:
            row = line.split('#')
            codepoint = int(row[0].split()[0], 16)
            descr = row[1].strip()
            self.iso8859_1_dict[codepoint] = descr
    
    def isascii(self, cp):
        return cp < 128
    
    def iscontrol(self, cp):
        if cp < 0x20: # SPACE
            return True
        if cp > 0x7F and cp < 0xA0:
            return True
        return False
    ...
    def isSpecialCharacter(self, cp):
        SpecialCharacters = r" # & ' ( ) * + , - . / : ; < = > [ ] _ |".split()
        if chr(cp) in SpecialCharacters: 
            return True
        return False
    ...
    def isOtherSpecialCharacter(self, cp):
        if cp < 0x20: # SPACE
            return False
        if self.iscontrol(cp):
            return False
        if self.isSpecialCharacter(cp): 
            return False
        if chr(cp).isalnum() or chr(cp).isspace():
            return False
        return True
    ...
    def as_cxx_hex_format(self, adict):
        alist = []
        for cp, descr in adict.items():
            if gen.isascii(cp):
                alist.append(json.dumps(chr(cp)).strip('"'))
            else:
                alist.append('\\x{:x}'.format(cp))
        return "".join(alist)   

    def as_cxx_string(self, adict):
        alist = []
        for cp, descr in adict.items():
                alist.append(chr(cp))
        return "".join(alist)   
...
if __name__ == "__main__":
    gen = Iso8859_1_Gen()
    UppercaseLetters = gen.as_cxx_hex_format(gen.classify(gen.isupper))
    LowercaseLetters = gen.as_cxx_hex_format(gen.classify(gen.islower))
    Digits = gen.as_cxx_hex_format(gen.classify(gen.isdigit))
    SpecialCharacters = gen.as_cxx_hex_format(gen.classify(gen.isSpecialCharacter))
    OtherSpecialCharacters = gen.as_cxx_hex_format(gen.classify(gen.isOtherSpecialCharacter))
    
    print(UppercaseLetters)
    print(LowercaseLetters)
    print(Digits)
    print(SpecialCharacters)
    print(OtherSpecialCharacters)
    print(gen.as_cxx_string(gen.classify(gen.isOtherSpecialCharacter)))
