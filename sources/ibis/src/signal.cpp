...
/*
 * https://gist.github.com/leo-yuriev/06b60804f99d33f11cff
 * https://github.com/qgis/QGIS/blob/master/src/app/main.cpp   !!!!
 *
 * https://www.boost.org/doc/libs/1_67_0/doc/html/boost_process/tutorial.html
 */
bool register_gdb_signal_handler();
static void gdb_signal_handler(int sig, siginfo_t *, void *);

bool gdb_detected();
bool valgrind_detected();
#if defined(BOOST_OS_LINUX)
bool token_in_procfs(std::string const& token, std::string const& procfs_path);
#endif
...
void gdb_signal_handler(int sig, siginfo_t *, void *)
{
    ...
    bp::child gdb_proc(
        ...
        bp::args = {
            ...
            getExecutablePath(),            // required symbol data in the executable
...
bool gdb_detected()
{
    ...
    /* Detect GDB by the mean of /proc/$PID/cmdline for "gdb" */
    {
        ...
        if (token_in_procfs("gdb", path.str())) {
            *result = true;
            return *result;
        }
    }
