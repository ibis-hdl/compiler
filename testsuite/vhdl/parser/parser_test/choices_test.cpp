...
struct choices_dataset : public testsuite::vhdl_parser::util::dataset_loader
{
    choices_dataset()
...
BOOST_DATA_TEST_CASE( choices,
    ...
    ^ choices_dataset.test_case_name(),
    input, expected, test_case_name)
{
    using attribute_type = ast::choices;
#if defined(NO_BOOST_SPIRIT_INSTANCES)
    auto const parser = parser::choices;
#else    
    auto const& parser = testsuite::vhdl_parser::choices();
#endif
