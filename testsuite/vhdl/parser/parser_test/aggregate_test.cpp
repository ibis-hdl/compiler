...
struct aggregate_dataset : public testsuite::vhdl_parser::util::dataset_loader
{
    aggregate_dataset()
    : dataset_loader{ "test_case/aggregate" }
    { }
} const aggregate_dataset;




BOOST_DATA_TEST_CASE( aggregate,
      aggregate_dataset.input()
    ^ aggregate_dataset.expect()
    ^ aggregate_dataset.test_case_name(),
    input, expected, test_case_name)
{
    using attribute_type = ast::aggregate;
#if defined(NO_BOOST_SPIRIT_INSTANCES)
    auto const parser = parser::aggregate;
#else    
    auto const& parser = testsuite::vhdl_parser::aggregate();
#endif
