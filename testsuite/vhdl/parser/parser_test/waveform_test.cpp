...
struct waveform_dataset : public testsuite::vhdl_parser::util::dataset_loader
{
    waveform_dataset()
...
BOOST_DATA_TEST_CASE( waveform,
    ...
    ^ waveform_dataset.test_case_name(),
    input, expected, test_case_name)
{
    using attribute_type = ast::waveform;
#if defined(NO_BOOST_SPIRIT_INSTANCES)
    auto const parser = parser::waveform;
#else    
    auto const& parser = testsuite::vhdl_parser::waveform();
#endif
