...
struct use_clause_dataset : public testsuite::vhdl_parser::util::dataset_loader
{
    use_clause_dataset()
...
BOOST_DATA_TEST_CASE( use_clause,
    ...
    ^ use_clause_dataset.test_case_name(),
    input, expected, test_case_name)
{
    using attribute_type = ast::use_clause;
#if defined(NO_BOOST_SPIRIT_INSTANCES)
    auto const parser = parser::use_clause;
#else    
    auto const& parser = testsuite::vhdl_parser::use_clause();
#endif
