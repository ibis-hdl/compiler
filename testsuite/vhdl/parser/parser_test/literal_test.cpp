...
struct literal_dataset : public testsuite::vhdl_parser::util::dataset_loader
{
    literal_dataset()
...
BOOST_DATA_TEST_CASE( literal,
    ...
    ^ literal_dataset.test_case_name(),
    input, expected, test_case_name)
{
    using attribute_type = ast::literal;
#if defined(NO_BOOST_SPIRIT_INSTANCES)
    auto const parser = parser::literal;
#else    
    auto const& parser = testsuite::vhdl_parser::literal();
#endif
