...
#include <ibis/tool/event_trace/event_trace.hpp>
...
#endif
#if 0
        AddThreadNameMetadataEvents(); // requires trace_event args as 'M' (metatdata)
    ...
    TraceEvent::storage_ptr ptr)
{
    ...
    // don't move the time capture point, so it's independed of the code path below
...
{
    {
        ...
        // ensure, that clearing data doesn't resize memory capacity unitentionally.
    ...
    // that allocator as a member that would be like a dependency injected custom allocatorwhich is
    ...
    auto const AppendEventsAsJSON = [](std::vector<TraceEvent> const& events, std::size_t start,
        ...
        for (std::size_t i = 0; i < count && (start + i) < events.size(); ++i) {
            // FixMe: This inserts an unintended ',' into JSON's output - what is the original
            // intend by gh-mainroach/chrome-event-trace's implementation??
            // if (i > 0) {
            //     out += ",,,";
            // }
...
void TraceLog::BeginLogging()
{
    ...
    json.objectBegin();
    json.attributeBegin("traceEvents");
    json.arrayBegin();
    ss << '\n';
...
void TraceLog::EndLogging()  // -- maybe later static
{
    ...
    json.arrayEnd();
    // JSON Object Format, see
    // https://docs.google.com/document/d/1CvAClvFfyA5R-PhYUmn5OOQtYMH4h6I0nSsKchNAySU/preview
    json.attribute("displayTimeUnit", "ns");
    json.attributeEnd();
    json.objectEnd();
...
void TraceLog::AddThreadNameMetadataEvents()
{
    for (auto const id : thread_ids_seen) {
        ...
        logged_events_.emplace_back(                     // TraceEvent(...)
            id, clock::now(),                            // -- thead_id, time point
...
void TraceLog::OutputCallback(std::string_view out)
{
    ...
    #6 0x541dbc in TraceLog::~TraceLog() /home/olaf/work/CXX/ibis/source_instrumentation/build/../source/common/include/ibis/tool/event_trace/trace_log.hpp:11:7
